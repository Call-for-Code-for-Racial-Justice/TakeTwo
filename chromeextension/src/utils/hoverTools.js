...
$.get(chrome.extension.getURL('hoverTools.html'), function (data) {
    ...
    var categoriesEl = hoverToolEl.find('#highlighter--hover-tools--container').eq(0);
    console.log(categoriesEl);

    chrome.runtime.sendMessage({ action: "getCategories" }, function (response) {
        response.data.forEach(cat => {
            $(categoriesEl).append('<div id="' + cat.name + '" class="highlighter--category" style="background-color:' + cat.colour + '">' + cat.name + '</div>').on('click', '#' + cat.name, function () { 
                ...
                highlights.css('backgroundColor', cat.colour);
                update(highlightId, window.location.hostname + window.location.pathname, window.location.pathname, cat.colour, cat.name); // update the value in the local storage
            });
            // buttons.push({title: cat.name, icon: chrome.extension.getURL('images/'+cat.name+'.svg'), callback: callbackFunction})
        });
    });
...
function onDeleteBtnClicked(e) {
    ...
    update(highlightId, window.location.hostname + window.location.pathname, window.location.pathname, 'inherit', null); // update the value in the local storage
